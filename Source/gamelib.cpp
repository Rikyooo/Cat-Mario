/*
 * gamelib.cpp: 本档案储存支援游戏相关的class的implementation
...
/////////////////////////////////////////////////////////////////////////////
// CAnimation: 提供动画的能力
// 1. 要懂得怎么呼叫(运用)其各种能力，但是可以不懂下列的程式是什么意思
// 2. 自己写到运用CMovingBitmap的程式时，可以参考下列程式的写法
// 3. 如果需要修改或扩充CAnimation的功能时，请用继承或直接拷贝的方式，最好
//    不要直接改CAnimation。
/////////////////////////////////////////////////////////////////////////////
...
// 这个class提供可以移动的图形
// 要懂得怎么呼叫(运用)其各种能力，但是可以不懂下列的程式是什么意思
/////////////////////////////////////////////////////////////////////////////
...
void CGameState::ShowInitProgress(int percent)
{
    ...
    CBrush* pb, b(RGB(0,0,0));				   // 画黑色 progress框
    pb = pDC->SelectObject(&b);
    pDC->Rectangle(x1,y1,x2,y2);
    CBrush b1(RGB(255, 255, 255));				// 画白色 progrss中心
    pDC->SelectObject(&b1);
    pDC->Rectangle(progress_x1,progress_y1,progress_x2_end,progress_y2);
    CBrush b2(DEFAULT_BG_COLOR);					// 画蓝色 progrss进度
    pDC->SelectObject(&b2);
    pDC->Rectangle(progress_x1,progress_y1,progress_x2,progress_y2);
    pDC->SelectObject(pp);						// 释放 pen
    pDC->SelectObject(pb);						// 释放 brush
    CDDraw::ReleaseBackCDC();					// 放掉 Back Plain 的 CDC
    //
    // 如果是别的地方用到CDC的话，不要抄以下这行，否则荧幕会闪烁
    //
...
/////////////////////////////////////////////////////////////////////////////
// CGame: Game Class
// 这个class是游戏的facade，是MFC与各个游戏状态的桥梁，如果不增加或减少
// 游戏状态的话，可以不用管这个class的介面与实作。
/////////////////////////////////////////////////////////////////////////////

CGame CGame::instance;

CGame::CGame(): NUM_GAME_STATES(3)
{
    running = true;
    suspended = false;
...
bool CGame::OnIdle()  // 修改功能不要修改OnIdle()，而应修改OnMove()及OnShow()
{
    if (suspended)
    {
        running = false;
    ...
    //
    // 以下的程式控制游戏进行的速度，注意事项：
    // 1. 用Debug mode可以检视每一次回圈花掉的时间，令此时间为t。
    // 2. 从上次离开OnIdle()至此，时间定为33ms，不可删除，其时间不可低于t。
    //
    ...
    CSpecialEffect::SetCurrentTime();	// 设定离开OnIdle()的时间
    return true;
}

void CGame::OnInit()	// OnInit() 只在程式一开始时执行一次
{
    //
    // 启动乱数
    //
    ...
    // 开启DirectX绘图介面
    //
    CDDraw::Init(SIZE_X, SIZE_Y);							// 设定游戏解析度

    //
    // 开启DirectX音效介面
    //
	CAudio::Instance()->Open();					// 开启音效介面
    //if (!CAudio::Instance()->Open())	
    //    AfxMessageBox("Audio Interface Failed (muted)");	// 无音效介面
...
// 这个class的全部程式都是低阶的绘图介面，可以全部略过不看
/////////////////////////////////////////////////////////////////////////////
