...
/////////////////////////////////////////////////////////////////////////////
// 这个class会建立DirectX Audio物件，以提供音效
// 每个Public Interface的用法都要懂，Implementation可以不懂
// 注意事项：音效档案可以用WAVE档(.wav)或MIDI档(.mid)，数个音效档案可以
//           混合(同时)拨放，但是其中只能混合一个MIDI档，两个MIDI档案不
//           能同时拨放，如果同时拨放两个MIDI档的话，前一个MIDI档会被停
//           掉。
/////////////////////////////////////////////////////////////////////////////
...
public:
	~CAudio();
	void           Close();						// 关闭Direct Sound介面
	static CAudio* Instance();					// 取得CAudio的Instance
	bool           Load(unsigned, char *);		// 载入编号i的声音，指定MIDI档案
	bool           Open();						// 开启Direct Sound介面
	void		   Pause();						// 暂停播放所有音效
	void           Play(unsigned, bool=false);	// 开始拨放编号i的声音，指定是否重覆拨放
	void		   Resume();					// 复原暂停播放的音效
	void           SetPowerResume();			// 电源恢复
	void           Stop(unsigned);				// 停止拨放编号i的声音
private:
	class Info {
	public:
		Info() {
	...
	static CAudio		audio;		// 自动destruct
	map<int, Info>		info;
	bool				isOpened;
